...
import { getServerAgentClient } from '@/lib/agent-server-client';
...
jest.mock('@/lib/agent-server-client', () => ({
  getServerAgentClient: jest.fn().mockReturnValue({
    createThread: jest.fn().mockResolvedValue({ thread_id: 'test-thread-id' }),
    client: {
      runs: {
...
%%EOF`;
  ...
  it.skip('should call the ingestion graph with the correct data', async () => {
    ...
    expect(getServerAgentClient().createThread).toHaveBeenCalled();
    expect(getServerAgentClient().client.runs.wait).toHaveBeenCalledWith(
      'test-thread-id',
      'document_ingest',
      {
        input: {
